...
def main():
    ...
    img = Image.open('./data/n01677366_common_iguana.JPEG')
    x_initial = preprocess(img)
    x_initial = torch.unsqueeze(x_initial, dim=0)

    print("INITIAL")
    predict(model, categories, x_initial, top_k=5)
    predict(vgg_model, categories, x_initial, top_k=5)

    y_trg = torch.tensor([0.0]*1000)
    trg_idx = 999
    print(f"\nTarget adversarial: {categories[trg_idx]}")
    y_trg[trg_idx] = 1.0
    y_trg = torch.unsqueeze(y_trg, dim=0)
    x_initial, x_adv = pgd_adversarial(x_initial, y_trg, model, regularization_factor=1000.0)
