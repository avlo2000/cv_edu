...
class App:
    ...
    def __init__(self, args, window, window_title, video_source=0):
        ...
        # click notifiers
        self.marker1 = tkinter.Label(self.control_frame, text="Click graph to add point!", width=19, borderwidth=2,
                                     relief="sunken")
        self.marker1.grid(row=1, column=2, padx=0, pady=5)
        self.marker2 = tkinter.Label(self.control_frame, text="Click graph to add point!", width=19, borderwidth=2,
                                     relief="sunken")
        self.marker2.grid(row=2, column=2, padx=0, pady=5)
        # calibrate button
        self.calbutton = tkinter.Button(self.control_frame, text="Calibrate", width=6, fg="yellow", bg="red",
                                        activebackground='red', command=self.calibrate)
        ...
        # Create a canvas that can fit the above video source size
        # weird, we have to sub 3 pixels to have it fit?
        self.canvas1 = tkinter.Canvas(self.bottom_frame, width=634, height=252, borderwidth=2, relief="sunken",
                                      cursor="tcross")
        ...
        # Peak hold
        self.peakholdbtn = tkinter.Button(self.bottom_frame, text="Peak Hold", width=6, fg="black", bg="yellow",
                                          activebackground='yellow', command=peakhold)
...
class MyVideoCapture:
    def __init__(self, calibration, video_source=0):
        ...
        self.vid.set(cv2.CAP_PROP_FRAME_WIDTH, 2592)
        self.vid.set(cv2.CAP_PROP_FRAME_HEIGHT, 1944)
        self.vid.set(cv2.CAP_PROP_BRIGHTNESS, 50)
        self.vid.set(cv2.CAP_PROP_EXPOSURE, 0.1)
        # Settings
        '''
        ...
        if self.vid.isOpened():
            ...
            if ret:
                ...
                for i in range(piwidth):
                    ...
                    if position != prevposition:  # because of rounding, sometimes we draw twice. Lets fix tht!
                        ...
                        if position % 50 == 0:
                            ...
                            cv2.putText(graph, str(position) + 'nm', (i - textoffset, 12), font, 0.4, (0, 0, 0), 1,
                                        cv2.LINE_AA)
                    zero += nmperpx
                ...
                for i in indexes:
                    ...
                    cv2.rectangle(graph, ((i - textoffset) - 2, height + 3), ((i - textoffset) + 45, height - 11),
                                  (0, 0, 0), 1)
                    cv2.putText(graph, str(wavelength) + 'nm', (i - textoffset, height), font, 0.4, (0, 0, 0), 1,
                                cv2.LINE_AA)
